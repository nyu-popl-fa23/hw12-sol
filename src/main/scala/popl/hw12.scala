...
object hw12 extends js.util.JsApp:
   ...
   * - adding "extends App" or "extends Application" to your hw12 object,
              ...
              case Some((mutp_f, tp_f)) =>
                (mut_f, mutp_f) match
                  case (MLet, MLet) if t_f == tp_f =>
                    fts + (f -> (MLet, t_f))
                  case _ =>
                    fts + (f -> (MConst, join(t_f, tp_f)))
      TObj(fts)
      
    /** JoinFunMeet */
    case (TFunction (ts1, tret1), TFunction (ts2, tret2)) if ts1.size == ts2.size =>
      val ts = ts1.lazyZip(ts2).map(meet)
      TFunction(ts, join(tret1, tret2))

    /** JoinNothing_1,2 */    
    case (TNothing, t) => t
    case (t, TNothing) => t

    /** JoinBasic, JoinAny_1, JoinAny_2 */
  ...
  def meet(t1: Typ, t2: Typ): Typ = (t1, t2) match
    ...
    /** MeetFunJoin */
    case (TFunction (ts1, tret1), TFunction (ts2, tret2)) if ts1.size == ts2.size =>
      val ts = ts1.lazyZip(ts2).map(join)
  ...
  def typeInfer(env: Map[String, (Mut, Typ)], e: Expr): Typ =
    ...
    def checkSubtyp(texp: Typ, e1: Expr): Typ =
      val tgot = typ(e1)
      if subtype(tgot, texp) then tgot else err(tgot, e1)
      ...
      case BinOp(bop, e1, e2) =>
        bop match
          ...
          case Eq | Ne => 
            ...
            else if hasFunctionTyp(t2) || join(t1, t2) == TAny then err(t2, e2)
            else TBool
